<template>
  ...
  <div :id="idStylePage">
    <h5 class="text-primary w600 mb-6 mt-3" style="--fs: 22px">Inspecciones</h5>
...
<script setup>
import '@/assets/styles/pages/birds-exit/inspection-transport-component.scss'
...
const idStylePage = "inspection-transport-component",
...
headers = [
  ...
  { title: "Id Salida", key: "idSalida", align: "center" },
  { title: "Lote", key: "lote", align: "center" },
  { title: "Id recepción", key: "idRecepcion", align: "center" },
  { title: "Orden Salida", key: "orden", align: "center" },
  { title: "Cantidad aves", key: "cantidad", align: "center" },
  { title: "Peso prom.", key: "pesoProm", align: "center" },
  { title: "Num. jaulas", key: "numJaulas", align: "center" },
  { title: "Fecha salida", key: "fechaSalida", align: "center" },
  { title: "Hora salida", key: "horaSalida", align: "center" },
  { title: "Estatus", key: "estatus", align: "center" },
  { title: "Origen", key: "origen", align: "center" },
  { title: "Destino", key: "destino", align: "center" },
  { title: "Transporte", key: "transporte", align: "center" },
  { title: "Placa", key: "placa", align: "center" },
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.granjas.getInspeccionesTransporte(data)

    pageCount.value = Math.ceil(Number(response[1]) / Number(itemsPerPage.value));

    itemsTable.value = response[0].map((item) => ({
      id: item.id,
      idSalida: item.salida_id.id,
      lote: item.lote,
      idRecepcion: props.etapa == "PRODUCCION" ? item.salida_id.lote_id.id : item.salida_id.recepcion_id.id,
      orden: item?.salida_id?.orden_id?.orden,
      cantidad: item.cant_aves_translado,
      pesoProm: item.peso_prom_salida,
      numJaulas: item.num_jaulas,
      fechaSalida: item.fecha_salida,
      horaSalida: item.hora_salida,
      estatus: item.salida_id.status,
      origen: item.salida_id.granja_origen_id.name,
      destino: item.salida_id[props.etapa == "CRIA" ? "granja_destino_id" : "planta_destino_id"]?.name,
      transporte: `${item?.salida_id?.transporte_id?.id_sap} - ${item?.salida_id?.transporte_id?.name}`,
      placa: item.salida_id.transporte_id.id_sap,
      fechaRegistro: item.creation_date,
      user: `${item.user_creacion.nombre} ${item.user_creacion.apellido}`
    }));
