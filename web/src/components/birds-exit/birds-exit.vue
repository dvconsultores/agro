...
<script setup>
import '@/assets/styles/pages/birds-exit/birds-exit-component.scss'
...
idStylePage = "birds-exit-component",
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.granjas.getSalidasAves(data)
    ...
    itemsTable.value = response[0].map((item) => ({
      id: item.id,
      lote: item.lote,
      idRecepcion: props.etapa == "PRODUCCION" ? item.lote_id?.id : item.recepcion_id?.id,
      orden: item?.orden_id?.orden,
      cantidad: item.cant_aves,
      fechaSalida: moment(item.fecha_salida).format('DD/MM/YYYY'),
      estatus: item.status,
      origen: item.granja_origen_id?.name,
      destino: item[props.etapa == "CRIA" ? "granja_destino_id" : "planta_destino_id"]?.name,
      transporte: `${item.transporte_id?.id_sap} - ${item.transporte_id?.name}`,
