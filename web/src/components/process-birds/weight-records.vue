<template>
  ...
  <div :id="idStylePage">
    <h5 class="text-primary w600 mb-6 mt-3" style="--fs: 22px">Registros Peso</h5>
...
<script setup>
import '@/assets/styles/pages/process-birds/weight-records.scss'
...
headers = [
  ...
  { title: "Cantidad aves", key: "cantidadAves", align: "center" },
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.granjas.getResgistrosPesaje(data)
    ...
    itemsTable.value = response[0].map((item) => ({
      ...
      granja: `${item.granja_id.name} - ${item.granja_id.id_sap}`,
      galpon: item.galpon_distribucion_id.galpon_id.galpon,
      cantidadAves: item.cantidad,
      pesoProm: item.peso_prom,
      fecha: moment(item.fecha).format('DD/MM/YYYY'),
      fechaRegistro: moment(item.fecha_registro).format('DD/MM/YYYY'),
