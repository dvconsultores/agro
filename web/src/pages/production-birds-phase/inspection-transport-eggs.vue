...
idStylePage = "inspection-transport-eggs",
pageTitle = "Inspección transporte huevos",
pageSubtitle = "Inspección transporte huevos producción",
toast = useToast(),
currentTab = ref(0),
showTable = ref(true),
...
headers = [
  ...
  { title: "Cantidad huevos", key: "cantidad", align: "center" },
  ...
  { title: "Incubadora destino", key: "destino", align: "center" },
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.produccion.getInspeccionesTransporteHuevos(data)
    ...
    itemsTable.value = response[0].map((item) => ({
      ...
      lote: item.lote,
      idRecepcion: item.salida_id.lote_id.id,
      cantidad: item.cant_huevos_translado,
      pesoProm: item.peso_prom,
      fechaSalida: moment(item.fecha_salida).format('DD/MM/YYYY'),
      horaSalida: item.hora_salida,
      estatus: item.salida_id.status,
      origen: item.salida_id.granja_origen_id.name,
      destino: item.salida_id.incubadora_destino_id.name,
      ...
      fechaRegistro: moment(item.creation_date).format('DD/MM/YYYY'),
    ...
    loadingTable.value = false;
    toast.error(`Error el ejecutar getItemsTabla etapa '${etapa}': ${error}`);
    console.log(error)
  };
}
