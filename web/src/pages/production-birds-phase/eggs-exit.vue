<template>
  ...
  <div :id="idStylePage">
    <h5 class="text-primary w600 mb-6 mt-3" style="--fs: 22px">Salidas</h5>
...
<script setup>
import '@/assets/styles/pages/reproduction-breeding-phase/eggs-exit.scss'
...
import BirdsExit from '@/components/birds-exit/birds-exit.vue';


const
etapa = ApiLiderPollo.enums.EtapaGranjaEnum.PRODUCCION,
idStylePage = "eggs-exit",
pageTitle = "Salida huevos",
pageSubtitle = "Salida huevos producción",
toast = useToast(),
currentTab = ref(0),
showTable = ref(true),
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.produccion.getSalidasHuevos(data)
    ...
    itemsTable.value = response[0].map((item) => ({
      id: item.id,
      lote: item.lote,
      idRecepcion: item.id_recepcion,
      cantidad: item.cant_huevos,
      fechaSalida: moment(item.fecha_salida).format('DD/MM/YYYY'),
      estatus: item.status,
      origen: item.granja_origen_id.name,
      destino: item.incubadora_destino_id.name,
      transporte: item.transporte_id.name,
      placa: item.placa_transporte,
    ...
    loadingTable.value = false;
    toast.error(`Error el ejecutar GranjasProduccion.getSalidasHuevos : ${error}`);
    console.log(error)
  };
}
