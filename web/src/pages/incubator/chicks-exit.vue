...
subtitlePage = "Salidas pollitos BB",
toast = useToast(),
currentTab = ref(0),
showTable = ref(true),
loadingRegister = ref(false),
headers = [
  { title: "Id salida", key: "id", align: "center" },
  ...
  { title: "Cabtidad pollitos", key: "cantPollitos", align: "center" },
  ...
  { title: "Condición vehiculo", key: "condicionVehiculo", align: "center" },
  { title: "Granja destino", key: "granja", align: "center" },
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.incubadora.getSalidasPollitos(data)
    ...
    tablaItems.value = response[0].map((item) => ({
      ...
      cantPollitos: item.cant_pollitos,
      fechaSalida: new Date(item.fecha_salida).toLocaleDateString(),
      horaSalida: item.hora_salida,
      transporte: `${item.transporte_id.id_sap} - ${item.transporte_id.name}`,
      condicionVehiculo: item.condicion_vehiculo,
      granja: `${item.granja_destino_id.id_sap} - ${item.granja_destino_id.name}`,
    }));
    ...
    "orden": "4500013227",
    "cant_pollitos": 16400,
    "condicion_vehiculo": "ok",
    "fecha_salida": "2024-09-27T00:00:00.000Z",
    "hora_salida": "11:38:00",
    "creation_date": "2024-10-16T15:46:02.000Z",
    "user_creation": {
        "id": 4,
        "user_name": "test",
        "nombre": "test",
