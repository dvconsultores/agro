...
etapaSubtitle = "Inspección de Incubadora",
toast = useToast(),
currentTab = ref(0),
showTable = ref(true),
...
headers = [
  ...
  { title: "Incubadora", key: "incubadora", align: "center" },
  { title: "Ventilacion", key: "ventilacion", align: "center" },
  { title: "Temperatura", key: "temp", align: "center" },
  { title: "Humedad", key: "humidity", align: "center" },
  { title: "Fecha creacion", key: "fechaCreacion", align: "center" },
...
async function getInspeccionesGranjas(data = {
  ...
  try {
    ...
    const response = await ApiLiderPollo.incubadora.getInspeccionesIncubadora(data);
    
    pageCount.value = Math.ceil(Number(response[1]) / Number(itemsPerPage.value));

    inspections.value = response[0].map((item) => ({
      id: item.id,
      fechaRegistro: new Date(item.fecha_inspeccion).toLocaleDateString(),
      incubadora: `${item.incubadora_id.id_sap} - ${item.incubadora_id.name}`,
      ventilacion: item.ventilacion,
      temp: item.temperatura,
      humidity: item.humedad,
      fechaCreacion: new Date(item.creation_date).toLocaleDateString(),
      data: `${item.user_creation.user_name} - ${item.user_creation.nombre} ${item.user_creation.apellido}`,
    }));
