<template>
  <Navbar title="Usuarios" sub-title="Crear usuario" class="mb-4" />
  ...
  <div id="create-user">
    <h5 class="text-primary w600 mb-6" style="--fs: 22px">Nuevo usuario</h5>

    <v-card class="px-4 py-6">
      <h6 class="text-primary w600 mb-6">Registro de usuario</h6>
...
async function getData() {
  try {
    const response = await ApiLiderPollo.users.getRoles({ isActive: true });
  ...
  try {
    const response = await ApiLiderPollo.granjas.getGranjas({ all: true, status: "ACTIVO" });
...
async function handleIntegration() {
  ...
  try {
    ...
    await ApiLiderPollo.users.createUser(data.value);
