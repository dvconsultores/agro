...
subtitlePage = "Recepción de huevos",
toast = useToast(),
currentTab = ref(0),
showTable = ref(true),
...
headers = [
  ...
  { title: "Cabtidad huevos", key: "cantRecibido", align: "center" },
  { title: "temperatura huevos", key: "temperaturaHuevos", align: "center" },
  { title: "Transporte entrega", key: "transporte", align: "center" },
  { title: "placa", key: "placa", align: "center" },
  { title: "Granja origen", key: "granja", align: "center" },
...
async function getItemsTabla(data = {
  ...
  try {
    ...
    tablaItems.value = response[0].map((item) => ({
      ...
      lote: item.lote,
      estatus: item.status,
      user: `${item.user_creacion.user_name} - ${item.user_creacion.nombre} ${item.user_creacion.apellido}`,
      fechaDespacho: new Date(item.fecha_recepcion).toLocaleDateString(),
      cantRecibido: item.cant_recibido,
      temperaturaHuevos: item.temp_huevos,
      transporte: `${item.transporte_entrega_id.id_sap} - ${item.transporte_entrega_id.name}`,
      placa: item.placa_transporte,
      granja: `${item.granja_origen_id.id_sap} - ${item.granja_origen_id.name}`,
    }));
