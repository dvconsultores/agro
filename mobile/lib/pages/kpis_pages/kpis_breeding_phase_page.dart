...
class KpisBreedingPhasePage extends StatefulWidget {
  const KpisBreedingPhasePage({super.key});

  @override
  State<KpisBreedingPhasePage> createState() => _KpieProductionPhasePageState();
}

class _KpieProductionPhasePageState extends State<KpisBreedingPhasePage>
    with ThemesMixin {
  ...
  Future<void> getLote(granjaId) async {
    ...
    try {
      ...
      final lote = await apiLiderPollo.getLotesRecepcion(granjaId, "CRIA");
  ...
  Future<void> getSemanas(recepcionId) async {
    ...
    try {
      ...
      final semanas = await apiLiderPollo.getSemanasLote(recepcionId, "CRIA");
  ...
  Future<void> getKpisLote(int? semanaNum) async {
    ...
    try {
      final kpis = await apiLiderPollo.getKpisLote(
          selectedLoteController.value, semanaNum, "CRIA");
